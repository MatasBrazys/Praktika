...
export default function NetworkImporter({ surveyModel, config }: Props) {
    ...
    const processText = useCallback((raw: string) => {
        ...
        if (missingRequired.length) {
            setParseError(`Cannot map required columns: ${missingRequired.map(f => f.name).join(', ')}. Check column order or add a header row.`);
            return;
        }
                ...
                <div className="ni-panel">
                    ...
                    {validRows.length > 0 && !parseError && (
                        <div className="ni-footer">
                            ...
                            >
                                {imported ? `✓ Imported ${validRows.length} rows` : `Import ${validRows.length} valid row${validRows.length > 1 ? 's' : ''} →`}
                            </button>
                        </div>
                    )}
